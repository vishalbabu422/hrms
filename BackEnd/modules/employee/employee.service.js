...
const generateEmployeeCode = async (organizationId, transaction) => {
  ...
  if (lastEmployee && lastEmployee.employee_code) {
    const lastNumber = parseInt(
      lastEmployee.employee_code.replace(orgCode, ""),
      10,
    );
...
exports.createEmployee = async (payload) => {
  return await sequelize.transaction(async (t) => {
    ...
    const existingEmail = await Employee.findOne({
      where: { email: payload.email, organization_id: payload.organization_id },
      transaction: t,
    });
...
exports.updateEmployeeStatus = async (id, payload, user, isSuperAdmin) => {
  ...
  if (
    ...
    !allowedStatuses.includes(payload.account_status)
  ) {
...
exports.generateEmployeeCodePreview = async (organizationId) => {
  const organization = await Organization.findByPk(organizationId, {
    attributes: ["org_code"],
  });
  ...
  return `${organization?.org_code ?? "ORG NAME"}/${month}${year}/${nextEmployeeId}`;
};
