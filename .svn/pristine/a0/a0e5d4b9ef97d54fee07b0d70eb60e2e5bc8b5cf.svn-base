...
const getEmployeeById = catchAsync(async (req, res, next) => {
  ...
  const result = await employeeService.getEmployeeById(features.query);
...
const getEmployeeCodePreview = catchAsync(async (req, res, next) => {
  const organizationId = req.isSuperAdmin
    ? req.query.organization_id
    : req.user.organization_id;

  const employeeCode =
    await employeeService.generateEmployeeCodePreview(organizationId);
