...
import { validateBank } from '../../../validations/bankValidation'
  ...
  useImperativeHandle(ref, () => ({
    submit: async () => {
      try {
        ...
        const validationErrors = validateBank(employeeBank, 'Bank')
        ...
        if (!/^\d{9,18}$/.test(account_number)) {
          toast.error('Enter valid account number (9-18 digits)')
          return false
        }
        ...
        if (!micr_code.trim()) {
          toast.error('MICR code is required')
          return false
        }

        // MICR validation (9 digits)
        if (!/^\d{9}$/.test(micr_code)) {
          toast.error('MICR code must be 9 digits')
          return false
        }
