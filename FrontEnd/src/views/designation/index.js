...
const Index = () => {
  ...
  const fetchDesignation = async () => {
    try {
      ...
      const response = await api.get('admin/designation/index?models=empanelment', { params })
  ...
  const handleDelete = async (id) => {
    try {
      await api.delete(`/admin/designation/delete/${id}`)
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(14)
    doc.text('Designation List', 10, 10)

    designation.forEach((item, index) => {
      doc.text(`${index + 1}. ${item.designation} - ${item.type} - ${item.exp_in_years} yrs`, 10, y)
      y += 8
    })
  ...
  const handleExportExcel = () => {
    const formattedData = designation.map((item, index) => ({
      'Sr No': index + 1,
      'Empanelment No': item.empanelment?.empanelment_no || '-',
      Type: item.type,
      Designation: item.designation,
      Qualification: item.qualification,
      'Experience (Years)': item.exp_in_years,
    ...
    saveAs(fileData, 'designation-list.xlsx')
  }
  ...
  return (
    <CContainer fluid>
      <PageHeader
        ...
        onAddClick={() => navigate('/designation/add')}
                ...
                {loading ? (
                  ...
                  designation.map((item, index) => (
                    <CTableRow key={item.id}>
                      ...
                      <CTableDataCell>{item.empanelment?.empanelment_no || '-'}</CTableDataCell>
                      <CTableDataCell>{item.type}</CTableDataCell>
                      <CTableDataCell>{item.qualification}</CTableDataCell>
                      <CTableDataCell>{item.exp_in_years}</CTableDataCell>
                      <CTableDataCell>
                        {item.is_active ? (
                        ...
                        >
                          ...
                          <CTooltip content="View" placement="top">
                            <div
                              ...
                              onClick={() => navigate(`/designation/preview/${item.id}`)}
                            >
                          ...
                          <CTooltip content="Edit" placement="top">
                            <div
                              style={{ display: 'inline-block', cursor: 'pointer' }}
                              onClick={() => navigate(`/designation/edit/${item.id}`)}
                            >
