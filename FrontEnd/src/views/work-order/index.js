  ...
  return (
    <CContainer fluid>
      ...
      <PageHeader
        ...
        onAddClick={() => navigate('/work-order/add')}
            ...
            <CTable hover responsive align="middle" className="rounded overflow-hidden">
              ...
              <CTableBody>
                {loading ? (
                  ...
                  workOrder.map((item) => {
                    const isMilestone = item.type === 'MILESTONES_PROJECT_BASIS'

                    const hasDesignation =
                      Array.isArray(item.WoDesgnMappings) && item.WoDesgnMappings.length > 0
                    return (
                      <CTableRow key={item.id}>
                        <CTableDataCell>
                          {item.type === 'MANPOWER' && (
                            <span className="badge bg-primary">Manpower</span>
                          )}

                          {item.type === 'MANPOWER_PROJECT_BASIS' && (
                            <span className="badge bg-success">Manpower (Project Basis)</span>
                          )}

                          {item.type === 'MILESTONES_PROJECT_BASIS' && (
                            <span className="badge bg-warning text-dark">Milestone</span>
                          )}
